/*
    tests.cpp: Testing all test cases with the sequential version of matrix multiplication 
...
} // namesapce sequential

char* sequential::Tests::filename = new char[100];
...
main(int argc, char **argv) 
{
  ...
  runner.addTest(sequential::Tests::suite());
  strcpy(sequential::Tests::filename, argv[2]);
  if (argc == 3 && strcmp(argv[1], "-i") == 0) 
    {
      controller.addListener(&result);
      runner.run(controller);
      std::ofstream xmlFileOut("sequential_results.xml");
