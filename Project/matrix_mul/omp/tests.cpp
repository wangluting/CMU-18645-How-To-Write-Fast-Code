/*
    tests.cpp: Testing all test cases with the OpenMP version of matrix multiplication 
...
char* omp::Tests::filename = new char[100];
...
main(int argc, char **argv) 
{
  ...
  runner.addTest(omp::Tests::suite());
  strcpy(omp::Tests::filename, argv[2]);
  if (argc == 3 && strcmp(argv[1], "-i") == 0) 
    {
      controller.addListener(&result);
      runner.run(controller);
      std::ofstream xmlFileOut("omp_results.xml");
