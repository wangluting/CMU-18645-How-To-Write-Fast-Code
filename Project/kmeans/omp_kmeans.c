...
#pragma omp parallel for private(j) //lutingw
	for (i=0; i<numClusters; i++)
        for (j=0; j<numCoords; j++)
            clusters[i][j] = objects[i][j];

    /* initialize membership[] */
#pragma omp parallel for //lutingw
...
//#pragma omp parallel for  private(j,k)       //lutingw
            for (i=0; i<numClusters; i++) {
...
#pragma omp parallel for private(j)      //lutingw
        for (i=0; i<numClusters; i++) {
			float temp =(newClusterSize[i] > 0) ? 1.0/newClusterSize[i]:0;
            for (j=0; j<numCoords; j++) {
                if (temp > 0)
